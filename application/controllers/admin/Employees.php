...
class Employees extends MY_Controller
{
    ...
    public function detail()
    {
        ...
        $data = array(
            ...
            'proposer_name' => $result[0]->proposer_name,
            'policy_no' => $result[0]->policy_no,
            'policy_type' => $result[0]->policy_type,
            'policy_start_date' => $result[0]->policy_start_date,
            'policy_end_date' => $result[0]->policy_end_date,
            'covered_members' => $result[0]->covered_members,
            'sum_insured' => $result[0]->sum_insured,
    ...
    public function salary_overtime()
    {
        ...
        foreach ($overtime->result() as $r) {
            $current_amount = $r->overtime_rate * $current_rate;
            $data[] = array(
                ...
                $r->no_of_days,
                $r->overtime_hours,
                'USD: ' . $r->overtime_rate . '<br>' . $current_title . ': ' . number_format($current_amount, 2, '.', '')
            );
        }
...
//	codelover138@gmail.com
    ...
    public function mediclaim()
    {
        ...
        if ($this->input->post('type') == 'mediclaim') {
            /* Define return | here result is used to return user data and error for error message */
            $Return = array('result' => '', 'error' => '', 'csrf_hash' => '');
            $Return['csrf_hash'] = $this->security->get_csrf_hash();

            /* Server side PHP input validation */
            if ($this->input->post('proposer_name') === '') {
                $Return['error'] = $this->lang->line('xin_nominee_error_name');
            } else if (preg_match("/^(\pL{1,}[ ]?)+$/u", $this->input->post('proposer_name')) != 1) {
                $Return['error'] = $this->lang->line('xin_hr_string_error');
            } else if ($this->input->post('policy_no') === '') {
                $Return['error'] = $this->lang->line('xin_policy_error_no');
            } else if ($this->input->post('policy_type') === '') {
                $Return['error'] = $this->lang->line('xin_policy_error_type');
            } else if ($this->input->post('policy_start_date') === '') {
                $Return['error'] = $this->lang->line('xin_policy_error_start');
            } else if ($this->input->post('policy_end_date') === '') {
                $Return['error'] = $this->lang->line('xin_policy_error_end');
            } else if ($this->input->post('covered_members') === '') {
                $Return['error'] = $this->lang->line('xin_policy_error_member');
            } else if ($this->input->post('sum_insured') === '') {
                $Return['error'] = $this->lang->line('xin_policy_error_insured');
            }else if ($this->input->post('cb_amount') === '') {
                $Return['error'] = $this->lang->line('xin_policy_error_cb');
            }

            if ($Return['error'] != '') {
                $this->output($Return);
            }
            $name = $this->Xin_model->clean_post($this->input->post('proposer_name'));
            $policy_no = $this->Xin_model->clean_post($this->input->post('policy_no'));
            $policy_type = $this->Xin_model->clean_post($this->input->post('policy_type'));
            $policy_start_date = $this->Xin_model->clean_post($this->input->post('policy_start_date'));
            $policy_end_date = $this->Xin_model->clean_post($this->input->post('policy_end_date'));
            $covered_members = $this->Xin_model->clean_date_post($this->input->post('covered_members'));
            $cb_amount = $this->Xin_model->clean_date_post($this->input->post('cb_amount'));
            $sum_insured = $this->Xin_model->clean_date_post($this->input->post('sum_insured'));
            $id = $this->input->post('user_id');
            $data = array(
                'proposer_name' => $name,
                'policy_no' => $policy_no,
                ...
                'mediclaim_status' => 'Yes',
            );
            $result = $this->Employees_model->mediclaim_info($data, $id);
            if ($result == TRUE) {
                $Return['result'] = $this->lang->line('xin_success_update_mediclaim_info');
            } else {
                $Return['error'] = $this->lang->line('xin_error_msg');
            }
            $this->output($Return);
